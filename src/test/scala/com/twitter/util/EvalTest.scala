...
class EvalTest extends WordSpec {
  ...
  val EvalSingleInstance = new Eval
  ...
  "Evaluator" should {
    ...
    "inPlace('expression')" in {
      ...
      EvalSingleInstance.compile("object Doubler { def apply(n: Int) = n * 2 }")
      assert(EvalSingleInstance.inPlace[Int]("Doubler(2)") == 4)
      assert(EvalSingleInstance.inPlace[Int]("Doubler(14)") == 28)
      // New class API fails
      // val eval = new Eval
    ...
    "#include" in {
      val derived = EvalSingleInstance[() => String](
        TempFile.fromResourcePath("/Base.scala"),
    ...
    "toSource returns post-processed code" in {
      val derived = EvalSingleInstance.toSource(TempFile.fromResourcePath("/DerivedWithInclude.scala"))
